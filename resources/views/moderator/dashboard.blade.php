...
<div class="dashboard-container" style="padding: 24px; max-width: 1200px; margin: 0 auto;">
    ...
    <!-- Premium Header -->
    <div class="glass-panel" style="padding: 40px; border-radius: var(--radius-lg); margin-bottom: 32px; background: var(--accent-gradient); color: white; position: relative; overflow: hidden; box-shadow: var(--shadow-lg);">
        ...
        <div style="position: relative; z-index: 1;">
            <h1 style="font-size: 32px; font-weight: 800; margin-bottom: 12px; letter-spacing: -0.5px;">Forum Overview</h1>
            <p style="opacity: 0.9; font-size: 16px; font-weight: 500;">
                Welcome back, <strong>{{ auth()->user()->name }}</strong>. Here is a summary of the forum's current health and activity.
            </p>
        </div>
    </div>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 24px; margin-bottom: 40px;">
        ...
        <div class="stat-card">
            ...
            <div class="stat-value" style="color: #ef4444;">{{ $stats['reported_posts'] + $stats['reported_comments'] }}</div>
    ...
    <div style="display: grid; grid-template-columns: 1.5fr 1fr; gap: 32px;">
        ...
        <section>
            <h2 style="font-size: 20px; font-weight: 800; margin-bottom: 24px; color: var(--text-primary); display: flex; align-items: center; gap: 12px;">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M12 20l4-9-4-9-4 9 4 9z"></path></svg>
                Recent Unapproved Posts
            </h2>
            <div style="display: flex; flex-direction: column; gap: 16px;">
                @forelse($recent_pending as $post)
                <div class="glass-panel" style="padding: 20px; border-radius: var(--radius-md); display: flex; justify-content: space-between; align-items: center; border-left: 4px solid var(--accent-primary);">
                    <div>
                        <div style="font-weight: 700; color: var(--text-primary);">{{ $post->title }}</div>
                        <div style="font-size: 13px; color: var(--text-muted);">by u/{{ $post->user->username }} • {{ $post->created_at->diffForHumans() }}</div>
                    </div>
                    <a href="{{ route('admin.approvals') }}" class="btn-sm">Review</a>
                </div>
                @empty
                <div style="padding: 32px; text-align: center; color: var(--text-muted); border: 2.5px dashed var(--border-glass); border-radius: var(--radius-md);">
        ...
        <aside>
            <h2 style="font-size: 20px; font-weight: 800; margin-bottom: 24px; color: var(--text-primary);">Quick Actions</h2>
            <div class="glass-panel" style="padding: 24px; border-radius: var(--radius-md); display: flex; flex-direction: column; gap: 16px;">
...
<style>
    .stat-card {
        background: var(--bg-glass); border: 1px solid var(--border-glass); padding: 24px; border-radius: var(--radius-md); text-align: center;
        box-shadow: var(--shadow-sm); transition: transform 0.2s ease;
    }
    .stat-card:hover { transform: translateY(-4px); }
    .stat-label { font-size: 11px; font-weight: 900; text-transform: uppercase; color: var(--text-muted); margin-bottom: 8px; letter-spacing: 0.8px; }
    .stat-value { font-size: 32px; font-weight: 900; color: var(--text-primary); margin-bottom: 4px; }
    .stat-footer { font-size: 11px; font-weight: 700; color: var(--text-muted); opacity: 0.8; }

    .btn-sm {
        padding: 8px 16px; border-radius: var(--radius-pill); background: rgba(0,0,0,0.05); color: var(--text-primary);
        font-size: 12px; font-weight: 800; cursor: pointer; text-decoration: none; transition: all 0.2s ease;
    }
    .btn-sm:hover { background: var(--accent-primary); color: white; }
    ...
    .action-link:hover { background: rgba(0,0,0,0.02); }
    .action-icon {
