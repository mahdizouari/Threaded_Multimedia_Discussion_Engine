                        ...
                        @foreach($trendingPosts as $tPost)
                            <div class="trending-card-reddit" onclick="window.location='{{ route('posts.show', $tPost->id) }}'">
                                ...
                                <div class="overlay">
                                    ...
                                    <div class="footer">
                                        <img src="{{ $tPost->user->profile && $tPost->user->profile->avatar_path ? asset('storage/' . $tPost->user->profile->avatar_path) : 'https://api.dicebear.com/7.x/avataaars/svg?seed=' . ($tPost->user->username ?? $tPost->user->name) }}"
                    ...
                    @auth
                        @if(Auth::id() !== $post->user_id)
                            <a href="{{ route('messages.show', $post->user_id) }}" class="interaction-btn"
                                style="padding: 6px 12px; border-radius: var(--radius-pill); font-size: 11px;">
                    ...
                    <div class="action-group">
                        @if(Auth::check())
                            <form action="{{ route('react.post', $post->id) }}" method="POST" style="display:inline;">
                                ...
                                <button type="submit"
                                    class="action-btn upvote {{ Auth::check() && $post->reactions->where('user_id', Auth::id())->where('appreciation.type', 'TOP')->count() ? 'active' : '' }}"
                                    title="Top (Upvote)">
                        ...
                        <span
                            class="vote-count">{{ $post->reactions->where('appreciation.type', 'TOP')->count() - $post->reactions->where('appreciation.type', 'FLOP')->count() }}</span>
                        ...
                        @if(Auth::check())
                            <form action="{{ route('react.post', $post->id) }}" method="POST" style="display:inline;">
                                ...
                                <button type="submit"
                                    class="action-btn downvote {{ Auth::check() && $post->reactions->where('user_id', Auth::id())->where('appreciation.type', 'FLOP')->count() ? 'active' : '' }}"
                                    title="Flop (Downvote)">
                                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                        stroke-width="2">
