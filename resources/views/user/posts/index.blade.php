<x-app-layout page-title="Fil des publications">
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-600">
                    Style «&nbsp;feed&nbsp;» lisible : votez (Top / Flop), ouvrez une publication pour commenter — aligné sur votre énoncé et sur une présentation claire des informations.
                </p>
            </div>
            @auth
                <a
    ...
    <div class="py-4 sm:py-6">
        <div class="mx-auto max-w-3xl px-3 sm:px-4">
            ...
            @guest
                <div class="mb-4 rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm" role="region">
                    <a href="{{ route('login') }}" class="font-semibold text-forum-700 hover:underline">Connectez-vous</a>
                    pour voter, publier ou commenter.
                </div>
            @endguest
            ...
            @if ($posts->count())
                <ul class="space-y-2.5" role="list">
                    @foreach ($posts as $post)
                        ...
                        @endphp
                        <li>
                            <article class="flex overflow-hidden rounded-md border border-slate-200 bg-white shadow-sm transition hover:border-slate-300">
                                ...
                                <div class="min-w-0 flex-1 p-3 sm:p-4">
                                    <div class="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-slate-500">
                                        <span class="font-semibold text-forum-700">f/{{ Str::slug($post->category->label ?? 'general', '-') }}</span>
                                        <span aria-hidden="true">·</span>
                                        <span>Par <span class="font-medium text-slate-700">{{ $post->user->name ?? 'Inconnu' }}</span></span>
                                        <span aria-hidden="true">·</span>
                                        ...
                                        @auth
                                            @if (auth()->id() === $post->user_id && ! $post->is_approved)
                                                <span class="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-bold uppercase text-amber-800">Modération</span>
                                            @endif
                                        @endauth
                                    </div>
                                    <h3 class="mt-1.5 text-base font-semibold leading-snug text-slate-900 sm:text-lg">
                                        <a href="{{ route('posts.show', $post) }}" class="hover:text-forum-700 focus:outline-none focus-visible:underline">
                                            {{ $post->title }}
                                        </a>
                                    </h3>
                                    ...
                                    <div class="mt-3 flex flex-wrap items-center gap-3 text-xs font-semibold text-slate-500">
                                        <a href="{{ route('posts.show', $post) }}#commentaires" class="inline-flex items-center gap-1 rounded-full px-2 py-1 hover:bg-slate-100 hover:text-slate-800">
                                        ...
                                        <span class="text-slate-400" title="{{ $topCount }} Top · {{ $flopCount }} Flop">{{ $topCount }}↑ {{ $flopCount }}↓</span>
                                    </div>
                                </div>
                            </article>
                        </li>
            ...
            @else
                <div class="rounded-lg border border-dashed border-slate-300 bg-white py-14 text-center shadow-sm">
                    <p class="text-slate-600">Aucune publication pour le moment.</p>
                    @auth
                        <a href="{{ route('posts.create') }}" class="mt-3 inline-block text-sm font-bold text-forum-700 hover:underline">
